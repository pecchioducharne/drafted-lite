...
const Chat = () => {
  ...
  const [activeChatName, setActiveChatName] = useState(recipientName);
  const [messages, setMessages] = useState([]);
  const [newMessage, setNewMessage] = useState("");
  const [chats, setChats] = useState([]);
  const [activeChatId, setActiveChatId] = useState(recipientId);
  ...
  const handleChatClick = (chatId, chatName) => {
    ...
    setActiveChatName(chatName); // Update the active chat name
    setNewMessage(
      `Hi ${chatName}! You look like a great fit for a position we are hiring for. Would be great to set some time.`
    );
  };
  ...
  useEffect(() => {
    const fetchChats = async () => {
      ...
      const unsubscribe = onSnapshot(q, (snapshot) => {
        ...
        const uniqueChats = allMessages.reduce((acc, msg) => {
          const chatPartnerId =
          ...
          if (!acc.find((chat) => chat.id === chatPartnerId)) {
            acc.push({ id: chatPartnerId, name: chatPartnerName });
          }
          return acc;
        }, []);

        setChats(uniqueChats);
        setActiveChatId(recipientId || uniqueChats[0]?.id);
      });
    ...
    if (activeChatId) {
      ...
      const unsubscribe = onSnapshot(q, (snapshot) => {
        ...
        const filteredMessages = allMessages.filter(
          (msg) =>
            (msg.senderId === currentUserId &&
              msg.receiverId === activeChatId) ||
            (msg.senderId === activeChatId && msg.receiverId === currentUserId)
        );

        setMessages(filteredMessages);

        // Clear the newMessage if it's the first time opening the chat
        if (filteredMessages.length === 0) {
          setNewMessage(""); // Or set a default message if needed
        }
      });
  ...
  const sendMessage = async (e) => {
    ...
    if (newMessage.trim()) {
      await addDoc(collection(db, "chats"), {
        ...
        senderIdName: currentUserId,
        receiverIdName: recipientName,
        message: newMessage,
        timestamp: new Date(),
      });
      setNewMessage("");
    ...
    <div className="chat-container">
        ...
        <div className="chat-tabs">
          {chats.map((chat) => (
            <button
              ...
              onClick={() => handleChatClick(chat.id, chat.name)} // Use the new click handler
            >
              {chat.name}
            </button>
          ))}
      ...
      <div className="chat-main">
        <div className="chat-header">
          <h3>{activeChatName}</h3> {/* Update to use activeChatName */}
          <div className="email-address-container">
            <p className="email-address">{recipientId}</p>
            <button
