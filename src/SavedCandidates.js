  ...
  useEffect(() => {
    const fetchSavedCandidates = async () => {
      try {
        ...
        if (recruiterDoc.exists() && recruiterDoc.data().savedCandidates) {
          const savedIds = recruiterDoc.data().savedCandidates;
          
          // Fetch each candidate's details
          const candidatesData = await Promise.all(
            savedIds.map(async (id) => {
              const candidateDoc = await getDoc(doc(db, "drafted-accounts", id));
              return { id, ...candidateDoc.data() };
            })
          );
  ...
  return (
    <div className="saved-candidates-page">
      <div className="top-bar">
        <div className="back-button" onClick={() => navigate('/viewer')}>
          <FiArrowLeft size={24} />
          <span>Back</span>
        </div>
